...
class DataSet(object):
    ...
    def save_geodataframe_to_shp(
            self, dataframe: geopandas.GeoDataFrame, resultName: str,
            analysisTask: TaskOrName = None, resultIndex: int = None,
            subdirectory: str = None, **kwargs) -> None:
        """Save a geopandas data frame to a shapely file stored in this dataset.
    ...
    def __init__(self, dataDirectoryName: str, 
        ...
        Args:
            ...
            microscopeChromaticCorrectionsName: the name of the microscope chromatic
                    ...
                    to acquire the images represented by this ImageDataSet
            microscopeIlluminationCorrectionsName: the name of the microscope illumination
        ...
        if microscopeIlluminationCorrectionsName is not None:
    ...
    def _load_illumination_corrections(self): 
        ...
        if os.path.exists(path):
            ...
            for key, val in pickle.load(inputFile).items():
                self.illuminationCorrections[key]["dark"] = val["dark"]
                self.illuminationCorrections[key]["flat"] = val["flat"]
            inputFile.close()
